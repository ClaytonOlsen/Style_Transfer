...
'''
def input_gram_matrix(input_tensor):
    batch_size, height, width, number = map(lambda i: i.value, image_feature.get_shape())
    input_shape = tf.shape(input_tensor)
    num_locations = tf.cast(input_shape[1]*input_shape[2], tf.float32)
    image_feature = tf.reshape(input_tensor, (batch_size, height * width, number))
    return tf.linalg.einsum('bijc,bijd->bcd', image_feature, image_feature) / num_locations
'''
